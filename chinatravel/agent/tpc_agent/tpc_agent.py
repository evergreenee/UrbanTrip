...
from chinatravel.agent.tpc_agent.prompts import (
...
class TPCAgent(BaseAgent):
    ...
    def symbolic_search(self, symoblic_query):
        ...
        success, plan, time_info = self.generate_plan_with_search(symoblic_query)
        ...
        if not time_info[0]:
            ...
            t_remaining = self.TIME_CUT - time_info[1]
            T = 1000.0
            T_min = 1.0
            alpha = 0.95
            try:
                n_temp_steps = math.log(T_min / T) / math.log(alpha)
                n_temp_steps = max(1, int(n_temp_steps))
            except ValueError:
                print("错误: T, T_min, 或 alpha 参数无效。")
                return

            total_iterations_affordable = t_remaining / time_per_iteration
            dynamic_max_iter = total_iterations_affordable / n_temp_steps
            dynamic_max_iter = max(1, int(dynamic_max_iter))

            sa = SA(**self.init_kwargs)
            config = {
                "T": T,
                "T_min": T_min,
                "alpha": alpha,
                "max_iterations_per_temp": dynamic_max_iter
            }
            success, plan = sa.run(symoblic_query, plan, config, t_remaining)
    ...
    def generate_plan_with_search(self, query):
        ...
        # 提取用户需求
        # 获取用户约束信息
        # constraints_json, requirement_list = self.extract_user_constraints(query)
        ...
        for go_i in ranking_go:
            ...
            for back_i in ranking_back:
                if time.time() > self.time_before_search + self.TIME_CUT:
                    ...
                    return True, self.default_plan, [True, time.time() - self.time_before_search]
                ...
                if query["days"] > 1:  # 如果天数大于 1，则需要考虑酒店
                    ...
                    for hotel_i in ranking_hotel:
                        ...
                        try:
                            ...
                            print("TimeOutError")
                            return False, {"error_info": "TimeOutError"}, [True, time.time() - self.time_before_search]
                        # exit(0)

                        # print(success, plan)
                        if success:
                            return True, plan, [False, time.time() - self.time_before_search]
                        else:
                            if time.time() > self.time_before_search + self.TIME_CUT:
                                self.default_plan["backtrack_count"] = self.backtrack_count
                                return True, self.default_plan, [True, time.time() - self.time_before_search]
        ...
        return False, {"error_info": "No solution found."}, [False, time.time() - self.time_before_search]
        ...
        while len(candidates_type) > 0:
            ...
            # 如果是午餐、晚餐或景点
                ...
                if poi_type in ["lunch", "dinner"]:
                    ...
                    top_candidates = candidate_res_filtered
                ...
                # 如果是景点
                    ...
                    top_candidates = candidate_attr_filtered
    ...
    def ranking_intercity_transport_go(self, transport_info, query):
        return list(range(len(transport_info)))
    ...
    def ranking_hotel(self, hotel_info, query):
        return list(range(len(hotel_info)))
    ...
    def select_next_poi_type(self, candidates_type, plan, poi_plan, current_day, current_time, current_position):
        ...
        # if time_compare_if_earlier_equal("08:30", current_time) and time_compare_if_earlier_equal(current_time, "09:00"):
        ...
        # if time_compare_if_earlier_equal("09:00", current_time) and time_compare_if_earlier_equal(current_time, "12:00"):
        ...
        # if time_compare_if_earlier_equal("15:00", current_time) and time_compare_if_earlier_equal(current_time, "19:00"):
    ...
    def extract_user_constraints(self, query):
        time_before = time.time()
        query_message = [{"role": "user",
                          "content": CONSTRAINT_EXTRACTION_INSTRUCTION.format(hard_logic=self.query['hard_logic_py'])}]
        ...
        constraints = None
        match = re.search(r'UserConstraints:\s*(\{.*\})', answer, flags=re.DOTALL)
        ...
        if match:
            ...
            try:
                ...
                print(f"JSON解析失败，提取到的内容是: {json_str}")
                constraints = None
        else:
            code_block_match = re.search(r'```json\s*(\{.*\})\s*```', answer, flags=re.DOTALL)
            if code_block_match:
                try:
                    constraints = json.loads(code_block_match.group(1))
                except json.JSONDecodeError:
                    pass
