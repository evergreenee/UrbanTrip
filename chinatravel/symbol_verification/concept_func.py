...
def innercity_transport_cost(transports, node=None):
    ...
    # print(f"[DEBUG] innercity_transport_cost: transports={transports} cost={cost}")
    return cost
...
def innercity_transport_type(transports):
    if len(transports) == 3:
        # print(transports[1]["mode"])
        return transports[1]["mode"]
    elif len(transports) == 1:
        # print(transports[0]["mode"])
...
#
# # 你已有的 evaluate_constraints_py 函数
# def evaluate_constraints_py(hard_logic_py, plan, func_dict, verbose=False):
#     results = []
...
#
#
# index = "20250323103623551478"
# # 读取 constraints（里面有 "hard_logic_py" 字段）
# constraints_file = f"../data/generated_5000/{index}.json"
# constraints_data = load_json(constraints_file)
# constraints = constraints_data["hard_logic_py"]  # 提取硬约束
#
# # 读取 plan
# plan_file = f"../../results/TPCAgent_deepseek_oracletranslation/{index}.json"
# plan = load_json(plan_file)
#
# # 执行约束并打印结果
# results = evaluate_constraints_py(constraints, plan, func_dict, verbose=True)
# for i, res in enumerate(results, start=1):
#     print(f"Constraint {i}: {res}")
